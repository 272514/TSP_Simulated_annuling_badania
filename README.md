...
Katalog `/research` zawiera pełne archiwum 85 zrzutów ekranu z przebiegu optymalizacji, dokumentujących każde z 5 uruchomień (*Run 1-5*) dla wszystkich wariantów badawczych. Archiwum zostało podzielone na 4 foldery zgodnie z parametrami testowymi z tabeli `tab_param`:

### 📂 Folder `T/` – Wpływ temperatury początkowej ($T_0$)
Badanie zachowania algorytmu przy stałej liczby miast $n=100$, chłodzeniu $\alpha=0.97$ i 100 próbach na krok.
...
Badanie tempa spadku temperatury przy ustawieniach bazowych ($n=100$, $T_0=10\,000$, 100 prób na krok).
...
Badanie wydajności obliczeniowej i jakości rozwiązań dla coraz większej liczby miast przy stałym budżecie 400 iteracji i 100 prób na krok.
* **N1:** $n = 50$ miast
* **N2:** $n = 150$ miast
* **N3:** $n = 200$ miast
...
Badanie wpływu intensywności przeszukiwania sąsiedztwa 2-opt w każdym kroku termicznym ($n=100$, $T_0=10\,000$, $\alpha=0.97$).
